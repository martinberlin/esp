...
void setup() {
  ...
  IntParameter param_timelapse( "timelapse",         "Timelapse in secs",  timelapse_sec);
  StringParameter param_slave_cam_ip("slave_cam_ip", "Slave cam ip/ping", slave_cam_ip);
  StringParameter param_upload_host("upload_host",   "API host for upload", upload_host);
  StringParameter param_upload_path("upload_path",   "Path to API endoint", upload_path);
...
void saveConfigCallback() {
  digitalWrite(ledStatus, HIGH);
    EEPROM.put(0, timelapse_sec);
    EEPROM.put(100, slave_cam_ip);
    EEPROM.put(200, upload_host);
    EEPROM.put(300, upload_path);
    if (EEPROM.commit()) {
...
void shutterPing() {
  // Attempt to read settings.slave_cam_ip
  WiFiClient client;
  ...
  while (client.available() == 0) {
    if (millis() - timeout > 5000) {
      Serial.println(">>> Client Timeout in ShooterPing()");
      client.stop();
      return;
    }
  }
